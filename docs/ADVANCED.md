...
|**OpenCC**|`opencc`| `OPENCC_CONFIG` | `s2twp` | Offline Simplified↔Traditional Chinese via [OpenCC](https://github.com/BYVoid/OpenCC). Install: `pip install opencc-python-reimplemented`. Configs: `s2t`/`s2tw`/`s2twp`/`s2hk`/`t2s`/… Override with `-s opencc:s2tw`. |
